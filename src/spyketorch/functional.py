...
def pad(input, pad, value=0):
    ...
    """
    return fn.pad(input, pad, mode='constant', value=value)
...
def get_earliest(spikes, potentials):
    earliest = (spikes.size(0) - spikes.sum(dim=0, keepdim=True)).long()
    # [0, array-length-1 = batchsize-1]
    earliest.clamp_(0, spikes.size(0)-1)
    # gathering values
    return potentials.gather(dim=0, index=earliest)
...
def pointwise_inhibition(thresholded_potentials):
    ...
    values, indices = torch.max(thresholded_potentials, dim=1, keepdim=True)
    # compute signs for detection of the earliest spike
    signed = values.sign()
    # finding winners (maximum potentials between early spikes)
    winners = get_earliest(signed, indices)
    signed_indices = signed[-1:, :, :, :]
    # generating inhibition coefficient
    coefficient = torch.zeros_like(thresholded_potentials[0]).unsqueeze_(0)
    coefficient.scatter_(1, winners, signed_indices)
    # applying inhibition to potentials (broadcasting multiplication)
    return torch.mul(thresholded_potentials, coefficient)
...
def get_k_winners(potentials, kwta=1, inhibition_radius=0, spikes=None):
    ...
    earliest = get_earliest(spikes, potentials)
    # propagating the earliest potential through the whole timesteps
    truncated_pot = spikes * earliest
    # summation with a high enough value at spike positions (maximum of summation potentials over timesteps)
    ...
    for _ in range(kwta):
        ...
        if max_val.item() != 0:
            ...
            if inhibition_radius != 0:
                rowMin = max(0, max_idx_unraveled[-2]-inhibition_radius)
                rowMax = min(total.size(-2),
                             max_idx_unraveled[-2]+inhibition_radius+1)
                colMin = max(0, max_idx_unraveled[-1]-inhibition_radius)
                colMax = min(total.size(-1),
                             max_idx_unraveled[-1]+inhibition_radius+1)
                total[:, rowMin:rowMax, colMin:colMax] = 0
        else:
            # No maximum value has been left :)
            break
    return winners
...
def local_normalization(input, normalization_radius, eps=1e-12):
    ...
    kernel = torch.ones(1, 1, normalization_radius*2+1, normalization_radius *
                        2+1, device=input.device).float()/((normalization_radius*2+1)**2)
    ...
    y.unsqueeze_(1)
    means = fn.conv2d(y, kernel, padding=normalization_radius) + \
        eps  # computes means
